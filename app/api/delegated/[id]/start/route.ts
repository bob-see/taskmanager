...
export async function POST(_req: Request, ctx: Ctx) {
  ...
  if (delegatedTask.assignedToUserId !== currentUser.id) {
    return Response.json(
      { error: "Only the assigned user can start work" },
      { status: 403 }
    );
  }

  if (delegatedTask.status !== "ACCEPTED") {
    return Response.json(
      { error: "Only accepted delegated tasks can be started" },
      { status: 409 }
    );
  }
  ...
  try {
    ...
    try {
      await prisma.taskNote.create({
        data: {
          ...
          content: `${formatUserName(currentUser)} started work on this delegated task.`,
        },
      });
    } catch (error) {
      console.error("Delegated task start note creation failed", {
    ...
    console.error("Delegated task start failed", {
      delegatedTaskId: delegatedTask.id,
      taskId: delegatedTask.taskId,
      assignedToUserId: currentUser.id,
      error,
    ...
    return Response.json({ error: "Could not start delegated task" }, { status: 500 });
  }
}

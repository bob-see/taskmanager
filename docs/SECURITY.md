...
Profile reorder and manual timesheet routes scope records to the session user. Reorder validates the complete submitted set against the authenticated owner's profiles before any update and returns only that owner's rows. Timer reads, start, and stop resolve ownership through the authenticated user's profiles; one active timer is allowed per user and different users may run timers simultaneously. The proxy remains defence in depth rather than the ownership boundary.
...
The 18 July 2026 Milestone 3 audit confirmed that production currently has zero
physical foreign keys and reproduced seven historical orphan classes without any
count change from the earlier baseline. The records are excluded from active
owner/membership query paths, so no current cross-user exposure was found. Some
inaccessible Space descendants and deleted-task notes may still contain historical
user content; remediation therefore requires backup and per-class approval. The
current count-only audit has no mutation mode.
...
The 18 July 2026 bootstrap investigation confirmed that a retired repository script
contained and printed a predictable administrator password, and that a production
administrator hash matched it at the time of inspection. The password was manually
rotated in TaskManager that day. The unsafe bootstrap and related one-off email
update scripts have been removed from the current tree; Git history was not
rewritten, so the exposed value is permanently retired and must never be reused.
This repository cleanup did not change production data or revoke sessions. Any JWT
issued before password rotation remains covered by the separate active
session-revocation debt below.
...
|---|---|---|
| The completed framework security milestone upgraded `next` and `eslint-config-next` from 16.1.6 to 16.2.10. The direct Next.js advisories from the 18 July 2026 baseline audit are cleared; Next.js's pinned PostCSS dependency still has a moderate advisory. | The patched framework removes the reported Proxy/Middleware bypass and other direct Next.js advisory classes. Route-local authorisation remains necessary and is tracked separately. | Automated, build, Proxy redirect, public-route, PWA asset, and minimum-Node checks passed. Complete authenticated login, Server Action, and mutation smoke checks before release; retain PostCSS under dependency/toolchain review. |
| Prisma CLI, Client and MariaDB adapter are aligned at 7.8.0. The 18 July 2026 upgrade removed the Prisma/Hono, Effect, Defu and Lodash/Chevrotain high-severity chains from the production audit; one moderate repeated-slash advisory remained through Prisma's CLI-only `@prisma/dev` → `@hono/node-server` path. | The remaining Prisma finding affects bundled development/CLI tooling rather than TaskManager's deployed adapter/client request path. It should still be monitored for an upstream patch. | Verify GitHub's current alert state separately and reassess when Prisma publishes a compatible patch. Do not downgrade Prisma or override the transitive server package without Prisma compatibility evidence. |
| A targeted 18 July 2026 lockfile refresh moved `flatted` to 3.4.2, `picomatch` to 4.0.5, `js-yaml` to 4.3.0, `@babel/core` to 7.29.7, and Tailwind's PostCSS copy to 8.5.19. No direct dependency, override, application code, or production data changed. | Those development-tool packages are patched in the current lockfile. At the review, the remaining Next.js/PostCSS, NextAuth/UUID, and Prisma CLI/Hono alerts were not safely removable without supported upstream parent releases; TaskManager does not process untrusted CSS/YAML/globs or call UUID's affected buffer APIs, and does not use Hono `serveStatic`. | Verify GitHub's current alert state separately and monitor supported Next.js, NextAuth/Auth.js, and Prisma releases. Do not force major downgrades/upgrades or unsupported transitive overrides solely to clear the count. |
| Profile reorder authenticates locally, validates the complete submitted set against the authenticated owner's profiles, and performs scoped validation, updates, and response reads in one transaction. | The confirmed cross-user read/mutation path is closed; inaccessible and unknown IDs receive the same generic validation response without partial updates. | Retain wrong-user tests and perform a two-account negative smoke check after deployment. |
| Timer active reads, start, and stop are scoped to the authenticated user's profiles. Per-user row locking and a conditional active-row update serialise same-user operations; different users may have simultaneous timers. | The confirmed cross-user timer path and global installation-wide timer constraint are closed. Concurrent stop attempts cannot finalise or log the same timer twice. | Retain owner/wrong-user/concurrency tests and manually verify two accounts after deployment. |
...
| Automated tests now cover production-core profile reorder and timer ownership, per-user concurrency, and Brisbane timer dates with transactional in-memory adapters, but not real Prisma route integration, Groups, delegated lifecycle permissions, Space roles, notifications, or admin actions. | The fixed rules have focused regression coverage, but adapter/query wiring still relies on type/build checks and manual two-account verification until a disposable MariaDB route harness exists. | Before expanding multi-user or collaborative behavior and whenever security-sensitive routes change. |
| `relationMode = "prisma"` has no physical FK backstop. Milestone 3 added a repeatable count-only audit and found seven unchanged historical orphan classes, all outside current owner/membership query paths. | No active cross-user exposure was found, but inaccessible historical content remains and future FK/schema work could be blocked. | P2 remediation before relation/destructive work: verified backup, per-class approval, dry-run/count assertions, idempotent transaction, post-audit and smoke checks. |
...
| JWT sessions have no per-user revocation/version mechanism, and password reset does not explicitly revoke existing tokens. Accounts also have no forced-password-change lifecycle or self-service password change. | A previously issued token—including one issued before the 18 July bootstrap-password rotation—may remain usable until normal NextAuth expiry, and administrator-led password management remains the only supported path. | Decide separately whether a credential incident warrants global session invalidation; add per-user revocation/versioning with account-lifecycle or self-service password work. |

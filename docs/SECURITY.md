...
| Prisma CLI, Client and MariaDB adapter are aligned at 7.8.0. The 18 July 2026 upgrade removed the Prisma/Hono, Effect, Defu and Lodash/Chevrotain high-severity chains from the production audit; one moderate repeated-slash advisory remains through Prisma's CLI-only `@prisma/dev` → `@hono/node-server` path. | The remaining Prisma finding affects bundled development/CLI tooling rather than TaskManager's deployed adapter/client request path. It should still be monitored for an upstream patch. | Reassess after GitHub recalculates alerts and when Prisma publishes a compatible patch. Do not downgrade Prisma or override the transitive server package without Prisma compatibility evidence. |
| A targeted 18 July 2026 lockfile refresh moved `flatted` to 3.4.2, `picomatch` to 4.0.5, `js-yaml` to 4.3.0, `@babel/core` to 7.29.7, and Tailwind's PostCSS copy to 8.5.19. No direct dependency, override, application code, or production data changed. | Four development-tool alerts should close after GitHub rescans. The remaining Next.js/PostCSS, NextAuth/UUID, and Prisma CLI/Hono alerts are not safely removable without supported upstream parent releases; TaskManager does not process untrusted CSS/YAML/globs or call UUID's affected buffer APIs, and does not use Hono `serveStatic`. | Monitor supported Next.js, NextAuth/Auth.js, and Prisma releases. Do not force major downgrades/upgrades or unsupported transitive overrides solely to clear the count. |

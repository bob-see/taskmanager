...
| Remaining dependency/toolchain maintenance | Active | P2 | Maintainability debt | Prisma 7.8 still brings one moderate `@hono/node-server` finding through `@prisma/dev`; Next.js/PostCSS and NextAuth/UUID remain separate production audit groups. Node types target 20 while the supported runtime minimum is 22.13.0. | Address each root dependency in an isolated reviewed milestone when a supported fix is available; do not use `npm audit fix --force` or transitive overrides merely to silence alerts. | No |
